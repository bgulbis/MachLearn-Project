...
library(doParallel)
registerDoParallel(core = Sys.getenv("NUMBER_OF_PROCESSORS") / 2)
```
...
The first step in pre-processing the new training data set will be to remove any near-zero covariates.
...
The next step will be to find and remove any highly-correlated predictors.
...
The final pre-processing steps will be to center and scale the data, and impute missing values using the k-nearest neighbor method.
...
The out of sample error for the final model created using the Random Forest method is `r (1 - cmRF$overall["Accuracy"]) * 100`%
