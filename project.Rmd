...
```{r}
test <- dplyr::select(testing, -(X:new_window), -problem_id)
test <- test[, -nzv]
test <- test[, -highCor]
#test$classe <- NA
...
predGBM <- predict(modelGBM, validate)
cmGBM <- confusionMatrix(predGBM, validate$classe)

predLDA <- predict(modelLDA, validate)
cmLDA <- confusionMatrix(predLDA, validate$classe)
...
```{r}
predDF <- data.frame(predRF, predGBM, classe=validate$classe)
mod.comb <- train(classe ~ ., data=predDF, method="rf")
pred.comb <- predict(mod.comb, validate)
confusionMatrix(pred.comb, validate$classe)
