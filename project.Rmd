...
near.zero <- nearZeroVar(training[,8:159], saveMetrics = TRUE)
nz <- training[,near.zero$zeroVar == FALSE]

```

```{r}
modelGLM <- train(classe ~ ., method="glm", data=nz[8:151], trControl=trainControl("repeatedcv"))
```

```{r}
library(rpart)
modelTree <- train(classe ~ ., method="rpart", data=nz[,8:151], trControl=trainControl("repeatedcv"))
